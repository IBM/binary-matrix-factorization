...
#include <math.h>
#include <cassert>
Alg5p::Alg5p(Vec<Vec<int> > &C, int k, Vec<Vec<int> > &Stranp, Vec<Vec<int> > &B){
    ...
    double minError = overall_error(C, Stranp, B);
    double error;
    do {
        for (int l=0; l < k; ++l){
            Vec<int> s;
            find_min_alg5p(B, Stranp_new, C, l, s);
            for(int i=0; i<n; i++)
                Stranp_new[l][i]=s[i];
        }
...
double Alg5p::overall_error(Vec<Vec<int> > &C, Vec<Vec<int> > &Stranp, Vec<Vec<int> > &B){
...
void Alg5p::find_min_alg5p(Vec<Vec<int> > &B, Vec<Vec<int> > &Stranp,
                   Vec<Vec<int> > &C, int l, Vec<int> &smin)
{
    ...
    for (int is=0; is<rowsS; ++is){
        int zeroOne=0;
        if ( cover_alg5p(C, SB, is, 1, B, l) < cover_alg5p(C, SB, is, 0, B, l) ) zeroOne=1;
        s[is] = zeroOne;
    }
    smin=s;
}
Vec<Vec<int> > Alg5p::product_alg5(Vec<Vec<int> > &Stranp, Vec<Vec<int> > &B, int colS){
    Vec<Vec<int> > SB;
    int rowsS=(int)Stranp[0].size();
    int rowsB=(int)B.size();
    int colsB=(int)B[0].size();
...
double Alg5p::cover_alg5p(Vec<Vec<int> > &C, Vec<Vec<int> > &SB, const int &is,
